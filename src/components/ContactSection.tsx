...
const ContactSection = () => {
  ...
  return (
    <section id="contact" className="py-24 px-6 max-w-7xl mx-auto scroll-mt-20">
      <p className="font-label text-[10px] tracking-[4px] uppercase mb-4 section-tag reveal-heading" style={{ color: '#D4F000' }}>{t('Get started', 'Začni')}</p>
      <h2 className="font-heading text-6xl md:text-7xl mb-16 reveal-heading" style={{ color: '#F0EDE6' }}>{t('START YOUR JOURNEY.', 'ZAČNI SVOJU CESTU.')}</h2>

      <div className="grid md:grid-cols-2 reveal-content glass-card overflow-hidden">
        <div className="p-8 md:p-10">
          <p className="font-body text-sm mb-8" style={{ color: '#AAAAAA' }}>
            {t("Fill in the form and we'll get back to you within 24 hours.", 'Vyplňte formulár a ozveme sa vám do 24 hodín.')}
          </p>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="font-label text-[10px] tracking-[3px] uppercase" style={{ color: '#888888' }}>{t('YOUR NAME', 'VAŠE MENO')}</label>
              <input type="text" value={name} onChange={e => setName(e.target.value)}
                className="w-full bg-transparent font-body text-sm py-2 focus:outline-none transition-colors"
            ...
            <div>
              <label className="font-label text-[10px] tracking-[3px] uppercase" style={{ color: '#888888' }}>{t('HOW TO REACH YOU?', 'AKO VÁS KONTAKTOVAŤ?')}</label>
            ...
            <div>
              <label className="font-label text-[10px] tracking-[3px] uppercase" style={{ color: '#888888' }}>{t('FORMAT INTEREST', 'ZÁUJEM O FORMÁT')}</label>
            ...
            <button type="submit" disabled={sent}
              ...
              style={{ background: '#D4F000', color: '#0E0E0F', letterSpacing: '0.15em', fontWeight: 700 }}
              onMouseEnter={e => { if (!sent) { e.currentTarget.style.background = '#C4E000'; e.currentTarget.style.boxShadow = '0 0 30px rgba(212,240,0,0.35)'; }}}
          ...
          <div className="space-y-0">
            {contacts.map(c => (
              <a key={c.label} href={c.href} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-3 py-4 transition-colors px-2 -mx-2"
                ...
                <span className="font-label text-xs tracking-[2px] uppercase w-24" style={{ color: '#888888' }}>{c.label}</span>
                <span className="font-body text-sm" style={{ color: '#AAAAAA' }}>{c.value}</span>
              </a>
            ))}
          </div>
          <p className="font-body text-xs mt-6" style={{ color: '#555555' }}>ivan.b@thaurusguru.com</p>
        </div>
      </div>
    </section>
  );
