      ...
      <div className="border border-border grid md:grid-cols-2 reveal-content">
        <div className="p-8 md:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="font-label text-[10px] tracking-[3px] uppercase text-[#666666]">{t('YOUR NAME', 'VAŠE MENO')}</label>
              <input type="text" value={name} onChange={e => setName(e.target.value)}
                className="w-full bg-transparent border-b border-border font-body text-sm text-foreground py-2 focus:outline-none focus:border-primary transition-colors placeholder:text-[#444444]"
                placeholder={t('Enter your name', 'Zadajte svoje meno')}
                required maxLength={100} />
            </div>
            <div>
              <label className="font-label text-[10px] tracking-[3px] uppercase text-[#666666]">{t('HOW TO REACH YOU?', 'AKO VÁS KONTAKTOVAŤ?')}</label>
            ...
            <div>
              <label className="font-label text-[10px] tracking-[3px] uppercase text-[#666666]">{t('FORMAT INTEREST', 'ZÁUJEM O FORMÁT')}</label>
            ...
            <button type="submit" disabled={sent}
              className={`w-full font-body text-xs font-bold tracking-[2px] uppercase py-3 transition-all ${sent ? 'bg-[hsl(73,100%,50%)] text-[#0C0C0C]' : 'bg-primary text-primary-foreground hover:opacity-90'}`}>
          ...
          <div className="space-y-0">
            {contacts.map(c => (
              <a key={c.label} href={c.href} target="_blank" rel="noopener noreferrer"
                ...
                <span className="font-label text-xs tracking-[2px] uppercase text-[#777777] w-24">{c.label}</span>
                <span className="font-body text-sm text-foreground">{c.value}</span>
              </a>
            ))}
          </div>
          <p className="font-body text-xs text-[#777777] mt-6">ivan.b@thaurusguru.com</p>
        </div>
      </div>
    </section>
  );
