    ...
    <section id="formats" className="py-24 px-6 max-w-7xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-[1px] bg-border reveal-content">
        ...
        <div className="bg-background p-8 md:p-10 flex flex-col reveal-card" style={{ transitionDelay: '0ms' }}>
          <span className="bg-secondary text-label font-label text-[10px] tracking-[1px] uppercase px-3 py-1 self-start mb-4">
            {t('Self-study', 'Samoštúdium')}
          </span>
          <h3 className="font-heading text-5xl text-foreground mb-1">EDUCATION</h3>
          <p className="font-body text-xs text-label tracking-[2px] uppercase mb-8">
          ...
          <div className="space-y-3 flex-1">
            {features.map((f, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <span className={`font-body text-sm ${eduActive[i] ? 'text-foreground' : 'text-inactive line-through'}`}>
                  {t(f.en, f.sk)}
                </span>
              </div>
            ))}
          ...
          <div className="mt-8 pt-6 border-t border-border">
            <p className="font-label text-[10px] tracking-[2px] uppercase text-dim mb-1">{t('Community access', 'Prístup do komunity')}</p>
            <p className="font-body text-sm text-subtle mb-4">{t('Monthly subscription', 'Mesačné predplatné')}</p>
